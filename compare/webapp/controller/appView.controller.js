sap.ui.define([
  ...
  "com/epiuse/project1/libs/moment"
],

  function (Controller,MessageToast,JSONModel,Model,momentjs) {
      "use strict";
      var resultToDraw = {
        json1: "",
        json2: "",
    ...
    var datecontainingArr = [];
    var datecontainingArr2 = [];
      ...
      //Useful to get coloumns which appears in both files(Common file attributes)
       ...
       getFormat: function(d){
        for (var prop in dateFormats) {
              if(moment(d,dateFormats[prop])._isValid == true && moment(d,dateFormats[prop])._pf.unusedTokens.length == 0){
        ...
        onCompare: function(oEvent) {
          ...
          ignoreZero = this.getView().byId("checkbox1").getSelected();
          ...
          onCompareDueToSuggested: function (oEvent) {
            ...
            oCSVModelCompare1 = Body1;
            oCSVModelCompare2 = Body2;
    

            if(oCSVModelCompare1.length <= 1){
               internLines = oCSVModelCompare1.oData.CSVBaseJson.toString().split('\n');
            }else{
               internLines = oCSVModelCompare1.split('\n');
            }

            if(oCSVModelCompare2.length <= 1){
               externLines = oCSVModelCompare2.oData.CSVCompareJson.toString().split('\n');
            }else{
               externLines = oCSVModelCompare2.split('\n');
            }
           
            
    
            this.reOrderCSV(internLines, externLines);
            this.SuggestCSVcompare(oCSVModelCompare1, oCSVModelCompare2, delimit_1, delimit_2);
          },
          ...
          SuggestCSVcompare: function (json1, json2, delimit_1, delimit_2) {
            ...
            if(ignoreZero == false){
              ...
              for(var j=0;j<csvCompHeadCnt;j++){
                for(var h=0;h<csvBaseHeadCnt;h++){

                  if(csvBaseHeadCnt == csvCompHeadCnt){

                    console.log("File Keys are of the same length");
            ...
            if (csvBaseKeysCnt >= 1 && csvCompareKeysCnt >= 1) {
             ...
             //START KEY SUGGESTION PROCESS:
                var headerKeys1 = JSON.stringify(Body1[0]);
                var headerKeys2 = JSON.stringify(Body2[0]);

                Body1.shift();
                Body2.shift();

                var keymapArr1 = headerKeys1.split(',');
                var keymapArr2 = headerKeys2.split(',');

                var possiblekeysA = [];
                var possiblekeysB = [];

                var stringpossiblekeysA ='';
                var stringpossiblekeysB ='';

                for(var t=0;t<Body1.length;t++){
                    for(var b=0;b<Body2.length;b++){
                        if(Body1[t][b]){
                        if(Body2[t][b]){
                        if(Body1[t][b] == Body2[t][b]){
                            possiblekeysA.push(t);
                            possiblekeysB.push(b);
                      
                        }}
                    }
                }}

                if(possiblekeysA.length != 0){
                    for(var m=0;m<possiblekeysA.length;m++){
                        if(stringpossiblekeysA == ''){
                            stringpossiblekeysA = keymapArr1[m];  
                        }else{
                            stringpossiblekeysA = stringpossiblekeysA + ','+keymapArr1[m]; 
                        }
                    }
                }
                if(possiblekeysB.length != 0){
                    for(var n=0;n<possiblekeysB.length;n++){

                        if(stringpossiblekeysB == ''){
                            stringpossiblekeysB = keymapArr2[n]; 
                        }else{
                            stringpossiblekeysB = stringpossiblekeysB + ','+keymapArr2[n];   
                        }
                ...
                stringpossiblekeysA = stringpossiblekeysA.replace('[','');
                stringpossiblekeysA = stringpossiblekeysA.replace(']','');
                stringpossiblekeysA = stringpossiblekeysA.replace(/['"]+/g, '');
                stringpossiblekeysA = stringpossiblekeysA.replace(',undefined', '');
                
                stringpossiblekeysB = stringpossiblekeysB.replace('[','');
                stringpossiblekeysB = stringpossiblekeysB.replace(']','');
                stringpossiblekeysB = stringpossiblekeysB.replace(/['"]+/g, '');
                stringpossiblekeysB = stringpossiblekeysB.replace(',undefined', '');

                var arryposkeyA = stringpossiblekeysA.split(',');
                var arryposkeyB = stringpossiblekeysB.split(',');

                that.getView().byId("KeySelection").setValue(arryposkeyA[0]+','+arryposkeyA[1]);
                that.getView().byId("RowSelection").setValue(arryposkeyB[0]+','+arryposkeyB[1]);
            }    
          ...
          onUploadBase: function(e) {
             ...
             reader.onload = function(oEvent) {
               ...
               that.checkFunc(fU.oFileUpload).then(function(r){
                ...
                var outfile = strCSV.replaceAll('ï»¿', '');
                that.getView().byId("FileOutBase").setText(outfile); 
           ...
           onUploadCompare: function(e) {
            ...
            reader.onload = function(oEvent) {
              ...
              that.checkFuncCompare(fU.oFileUpload).then(function(r){
               ...
               var outfile = strCSV.replaceAll('ï»¿', '');
               that.getView().byId("FileOutCompare").setText(outfile); 
       ...
       //   Variant_File = to do structure Variant file to store set readible parameters
        ...
        dynamicCSVcompare: function(json1,json2,delimit_1,delimit_2){
          ...
          function a(a) {
            if(a){
              for (var e = [], d = a, h = 0; h < d.length; h++) {
                  a = !1;
                  var k = d[h];
                  k.length && k[0] == f && (2 <= k.length ? k[k.length - 1] !== f ? a = !0 : 2 < k.length && k[k.length - 2] === b && (a = !0) : a = !0);
                  a && h !== d.length - 1 ? d[h + 1] = k + c + d[h + 1] : e.push(k)
          ...
          if(ignoreZero == true ){
            //new code used to loop through data and strip leading Zero's if the checkbox has been ticked or left selected
            for(var j=0;j<Body1.length;j++){
                for(var t=0; t<Body1[j].length;t++){
                    let numtest = isNaN(Body1[j][t]);
                    if(numtest == false){
                    //  Body1[j][t] = parseInt(Body1[j][t]);   
                    }
                    if(this.getFormat(Body1[j][t])!= null){
                      var temp = Date.parse(Body1[j][t].toString());
                      Body1[j][t] = temp.toString().substring(0,5);
                      datecontainingArr.push(Body1[0][t]);
                    }
                   ///passes the indication of a date back/  

                    
                }
            }
            datecontainingArr = datecontainingArr.filter(function(elem, index, self) {
              return index === self.indexOf(elem);
          })
            //new code used to loop through data and strip leading Zero's if the checkbox has been ticked or left selected
            for(var p=0;p<Body2.length;p++){
              for(var q=0; q<Body2[p].length;q++){
                  let numtest = isNaN(Body2[p][q]);
                  if(numtest == false){
                  //  Body2[p][q] = parseInt(Body2[p][q]);   
                  }
                  if(this.getFormat(Body2[p][q])!= null){
                    var temp = Date.parse(Body2[p][q].toString());
                    Body2[p][q] = temp.toString().substring(0,5);
                    datecontainingArr2.push(Body2[0][q]);
                  }

                  }
              }
              datecontainingArr2 = datecontainingArr2.filter(function(elem, index, self) {
                return index === self.indexOf(elem);
            })
       ...
       for(var m = 0; m <= result.csv.length-1; m++){
        ...
        if(result.csv[m].columns[n].data){
         ...
         //    
           ...
           // stringDiff = stringDiff + result.csv[m].columns[1].data;
            firstdiff = false;
           }  
            }
       
        
                //if(result.csv[m].columns[1]){
                //  stringDiff = stringDiff + result.csv[m].columns[1].data;
               //   stringkeys = stringkeys + Body1[m].toString();
               // }
